...
export default function FeedbackButton() {
  ...
  const trigger = {
    ...
    background: 'var(--card-bg)', border: '1px solid rgba(0,195,122,0.35)',
    borderRadius: 40, padding: '9px 18px', color: 'var(--t2)',
  ...
  const modal = {
    width: 420, background: 'var(--card-bg)',
    border: '1px solid var(--border)',
    borderRadius: 16, padding: 28, fontFamily: 'Inter, sans-serif',
    boxShadow: '0 24px 60px rgba(0,0,0,0.28)',
  }
  const label  = { fontSize: 12, fontWeight: 600, color: 'var(--t3)', marginBottom: 6, display: 'block', textTransform: 'uppercase', letterSpacing: '0.06em' }
  const input  = { width: '100%', background: 'var(--input-bg)', border: '1px solid var(--input-border)', borderRadius: 8, padding: '10px 12px', color: 'var(--input-text)', fontSize: 14, outline: 'none', fontFamily: 'Inter, sans-serif', boxSizing: 'border-box' }
  ...
  return (
    <>
      ...
      <button
        ...
        onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(0,195,122,0.35)'; e.currentTarget.style.color = 'var(--t2)' }}
      >
        ...
        <div style={overlay} onClick={e => { if (e.target === e.currentTarget) setOpen(false) }}>
          <div style={modal}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                <p style={{ margin: 0, fontSize: 17, fontWeight: 700, color: 'var(--t1)' }}>Report / Feedback</p>
                <p style={{ margin: '2px 0 0', fontSize: 12, color: 'var(--t3)' }}>We read every submission and fix things fast.</p>
              </div>
              <button onClick={() => setOpen(false)} style={{ background: 'none', border: 'none', color: 'var(--t3)', cursor: 'pointer', fontSize: 20, lineHeight: 1 }}>✕</button>
            </div>
            ...
            {sent ? (
              ...
              <div style={{ textAlign: 'center', padding: '24px 0' }}>
                ...
                <p style={{ color: 'var(--t3)', fontSize: 13, margin: 0 }}>We'll review this and fix it ASAP.</p>
                <button
              ...
              <form onSubmit={submit}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {TYPES.map(t => (
                      <button
                        ...
                        style={{
                          background: type === t.value ? 'rgba(0,195,122,0.12)' : 'var(--surface-bg)',
                          border: `1px solid ${type === t.value ? 'rgba(0,195,122,0.5)' : 'var(--border)'}`,
                          borderRadius: 8, padding: '8px 10px', cursor: 'pointer',
                          color: type === t.value ? '#00C37A' : 'var(--t2)',
                ...
                <p style={{ fontSize: 11, color: 'var(--t4)', textAlign: 'center', margin: '10px 0 0' }}>
