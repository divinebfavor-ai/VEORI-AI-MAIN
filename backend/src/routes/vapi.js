...
async function handleCallEnded(call, event) {
  ...
  // Deal, follow-up sequence, seller memory, callback, photos, missed-call text,
  // direct mail, campaign stats, number health - shared with the in-house engine.
  try {
    await require('../services/postCallPipeline').runPostCallActions({
      callRec: { ...callRec, outcome, duration_seconds: duration, transcript: finalTranscript },
      outcome,
      aiAnalysis,
    });
  } catch (e) {
    console.error('[Vapi] post-call pipeline error:', e.message);
  }
}
