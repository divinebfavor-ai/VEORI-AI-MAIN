...
// Preview version tag. Baked into every preview URL generated by
// synthesizePreviewToUrl. Bump this whenever the preview PIPELINE changes (model,
// tuning, pacing, ambience) - getVoiceLibrary() masks any cached URL that lacks
// the current tag, which forces the frontend to hit the preview endpoint and
// regenerate. That is what keeps "what you hear in the picker" honest: stale
// clips rendered with old settings can never be served again.
const PREVIEW_VERSION_TAG = 'preview-pv3';
...
async function getVoiceLibrary() {
  ...
  // Mask stale previews (rendered with OLD settings/model, or ElevenLabs stock
  // clips stamped by syncVoiceLibrary). The frontend short-circuits on a cached
  // voice_preview_url - nulling it here makes the ▶ button call the preview
  // endpoint, which regenerates with the CURRENT live-call sound (turbo model +
  // tuning + pacing + ambience) and re-caches under PREVIEW_VERSION_TAG.
  return data.map((v) => (
    v.voice_preview_url && !v.voice_preview_url.includes(PREVIEW_VERSION_TAG)
      ? { ...v, voice_preview_url: null }
      : v
  ));
}
...
const VOICE_TUNING = {
  ...
  // warm/grounded instead of performative. v2 pass ("work on all the female
  // voices"): the first correction (0.62/0.14) over-flattened her - grounded but
  // monotone. Ease stability back a touch and give style a little life so she
  // reads calm AND conversational, not sedated.
  uwJhTSUhU9LVyeRjWtiC: {
    stability: envNum('ELEVENLABS_TUNE_VEXA_STABILITY', 0.60),
    style:     envNum('ELEVENLABS_TUNE_VEXA_STYLE',     0.18),
  },
  ...
  // her and pull style down so she sits calm, warm, trustworthy. v2 pass: give
  // back a bit of prosody swing + expressiveness so she carries emotion cues
  // instead of reading every line at one level.
  hGQkZQUA5RiOXIw7P9iO: {
    stability: envNum('ELEVENLABS_TUNE_KIORA_STABILITY', 0.52),
    style:     envNum('ELEVENLABS_TUNE_KIORA_STYLE',     0.22),
  },
  // Angel (female, aVR2rUXJY4MTezzJjPyQ) - same female-voice correction as Kiora
  // (v2 pass included: a little more rise/fall + engagement, still grounded).
  aVR2rUXJY4MTezzJjPyQ: {
    stability: envNum('ELEVENLABS_TUNE_ANGEL_STABILITY', 0.52),
    style:     envNum('ELEVENLABS_TUNE_ANGEL_STYLE',     0.22),
  },
...
// human, and ambienceService supplies the ROOM: a shared office-tone bed is
// mixed under live speech + between utterances (mediaStreamServer) and into
// picker previews (synthesizePreviewToUrl), so the agent sounds like a person
// on a mic in a real room instead of a dead studio. A licensed/consented voice
// CLONE still stacks on top of all of this as a pure DROP-IN - no code change:
...
// Every knob here (pacing, emotion profiles, base settings, ambience) still
// applies to the clone, so it inherits all of this humanization automatically.
...
/**
 * Generate a voice-picker PREVIEW that sounds like a real live call.
 *
 * Parity by construction with the stream engine (what a seller actually hears):
 *   - STREAM_MODEL_ID (turbo), not the REST multilingual model
 *   - warm emotion swing + per-voice VOICE_TUNING (same resolution chain)
 *   - humanizePacing on the text (same breath/beat treatment)
 *   - the SAME ambience bed mixed under the voice + a short room lead-in/tail
 * Output is a WAV (PCM16 @16k) uploaded under PREVIEW_VERSION_TAG/ so
 * getVoiceLibrary() recognises it as current. Returns public URL or null.
 *
 * @param {string} text            preview script
 * @param {object} [opts]
 * @param {string} [opts.voiceId]  ElevenLabs voice_id
 * @returns {Promise<string|null>}
 */
async function synthesizePreviewToUrl(text, { voiceId } = {}) {
  const client = getClient();
  const vId = voiceId || process.env.ELEVENLABS_VOICE_ID || DEFAULT_VOICE_ID;
  if (!client || !vId || !text) {
    if (!client) console.warn('[ElevenLabs] synthesizePreviewToUrl skipped - no API key');
    return null;
  }
  try {
    const settings = applyVoiceTuning(voiceSettingsForEmotion('warm'), vId);
    const spoken = humanizePacing(text);
    const { data } = await client.post(
      `/text-to-speech/${vId}`,
      { text: spoken, model_id: STREAM_MODEL_ID, voice_settings: settings },
      {
        params: { output_format: 'pcm_16000' },
        responseType: 'arraybuffer',
        timeout: 30000, // preview scripts are longer than call lines
      },
    );
    let pcm = Buffer.from(data);
    if (pcm.length < 2) return null;

    // Mix the shared room bed under the voice (dry fallback if the bed fails -
    // a clean preview beats a 502).
    try {
      pcm = await ambience.mixPreviewPcm16(pcm, 16000);
    } catch (err) {
      console.warn('[ElevenLabs] preview ambience mix failed (serving dry):', err.message);
    }

    const wav = ambience.pcm16ToWav(pcm, 16000);
    const path = `${PREVIEW_VERSION_TAG}/${vId}-${Date.now()}.wav`;
    const { error: uploadErr } = await supabase.storage
      .from(TTS_BUCKET)
    ...
    const detail = err.response?.data ? Buffer.from(err.response.data).toString('utf8').slice(0, 200) : err.message;
    console.warn('[ElevenLabs] synthesizePreviewToUrl failed:', detail);
    return null;
  }
}
